...
function createTable()
{
    ...
    for(let i=0; i<rows; i++){
        table += '<td><input type="number" style="width: 4em" id="modality'+i+'" value="'+(i+1)+'"></td>';
    }

    table += '</tr><tr><th>Effectif</th>';

    for(let i=0; i<rows; i++){
        table += '<td><input type="number" style="width: 4em" id="effectif'+i+'" value="4"></td>';
    }

    table += '</tr></table><div id="tableSettings"> <p>ECC (Effectif Cumulé Croissant)<input type="checkbox" id="ECCA"></p> <p>ECD (Effectif Cumulé Décroissant)<input type="checkbox" id="ECDA"></p> <p>Fréquence<input type="checkbox" id="frequencyA"></p> <p>Pourcentage<input type="checkbox" id="percentA"></p> <p>PCC (Pourcentage Cumulé Croissant)<input type="checkbox" id="PCCA"></p> <p>PCD (Pourcentage Cumulé Déroissant)<input type="checkbox" id="PCDA"></p> <p>Moyenne des effectifs<input type="checkbox" id="averageA"></p> <p>Moyenne des modalités<input type="checkbox" id="averageAmodality"></p> <p>Mode<input type="checkbox" id="modeA"></p> <p>Médiane<input type="checkbox" id="medianA"></p> </div>'
...
function generateTable()
{
    ...
    if(document.getElementById('averageAmodality').checked == true){
        table += '<tr> <th>Moyenne de "'+document.getElementById('modalityName').value+'"</th> <td colspan="'+(modality.length+1)+'" style="text-align:center" id="averagemodality">...</td> </tr>';
    }

    if(document.getElementById('averageA').checked == true){
        table += '<tr> <th>Moyenne des effectifs</th> <td colspan="'+(modality.length+1)+'" style="text-align:center" id="average">...</td> </tr>';
    }
...
function generate(){
    ...
    {
        for(let i=0; i<modality.length; i++){
            nA += (parseFloat(modality[i].value)*parseFloat(effectif[i].value));
            nAm += parseFloat(effectif[i].value);
        }
        nA /= nAm;
        document.getElementById('averagemodality').innerHTML = parseFloat(nA)
    }
